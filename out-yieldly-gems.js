...
(async () => {
    for (let i = 0; i < 3; i++) { // TRY TO RUN THE SCRIPT 3 TIMES TO BYPASS POSSIBLE NETWORK ERRORS
        try {
            log(`------ START -----`);
            log(`YIELDLY - 1/2 GEMS - 1/2 YLDY/ALGO Unstake${DEBUG ? " => [DEBUG] No transactions will be made!" : ""}`)
            ...
            const unStakedInGEMSAmount = await unStakeYLDY(browser, 393388133);
            log(`Un-Staked amount in YLDY-GEMS: ${unStakedInGEMSAmount} YLDY`);
